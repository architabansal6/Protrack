...
class NewRequest: BaseViewController, UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIActionSheetDelegate,MKMapViewDelegate,CLLocationManagerDelegate,UITextFieldDelegate{
        ...
        @IBAction func resSubmit()
        {
            ...
            else
            {
                ...
                var reqId = "Req"+String(arc4random()%10000)
                let dateTime = getCurrentDateAndTime()

                var url:NSURL = NSURL(string: "https://TelcoApp.mybluemix.net/task")!
                
                
                var request : NSMutableURLRequest = NSMutableURLRequest(URL: url)
                
                var keys:NSArray=NSArray(objects: "_id","desp","street","city","area","date","time","status")
                var objects:NSArray=NSArray(objects: reqId,reqDesp.text,street.text,city.text,area.text,dateTime.d,dateTime.t,"Open")
